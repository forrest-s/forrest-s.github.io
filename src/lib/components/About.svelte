...
>
  <div class="w-full flex-1 md:w-1/2">
    <img
      src={width > 430 ? info.desktopImg : info.mobileImg ? info.mobileImg : info.desktopImg }
      alt={info.title}
      class="w-full h-full max-h-[800px] object-cover"
    />
  </div>
...
<style>
  .red-after::after {
    background: theme(colors[secondaryAccentColor]);
  }
  .yellow-after::after {
